...
on setBackupAudioDevice()
	SetAudioDevice("MacBook Pro Speakers", "48,000 Hz", true)
end setBackupAudioDevice

SetAudioDevice("HIFI DSD", "24-bit Integer (96.0 kHz)", false)
on SetAudioDevice(myDeviceName, audioFormat, backupAlreadyFixed)
	...
	tell application "System Events"
		...
		tell process "Audio MIDI Setup"
			...
			tell outline 1 of scroll area 1 of splitter group 1 of window "Audio Devices"
				...
				if N = 0 then
					if backupAlreadyFixed then
						return
					end if
					
					return my setBackupAudioDevice()
				end if
